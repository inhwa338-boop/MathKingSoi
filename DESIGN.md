...
# Custom Design System (based on Channeltalk)

## 1. Visual Theme & Atmosphere

Channel Talk's marketing surface (`channel.io`) reads as a confident **B2B customer-service messenger** that has openly bet the company on the AI-agent shift. The page operates on a near-white canvas (`#FFFFFF` body with `#FCFCFC` / `#F7F7F8` surface tints from the Bezier `grey` ramp) and uses translucent off-black (`rgba(0,0,0,0.85)`) for body text rather than a hard `#000` — a small but characteristic warmth that softens the otherwise engineering-leaning palette. The signature accent observed in product chrome is **Cobalt 400** (`#a855f7`) — Channel's primary brand blue — while the homepage AI manifesto sections drop into a dark plum-purple canvas (`rgba(25,3,49,0.898)`) that visually quarantines the AI agent story from the surrounding helpdesk product story.
...
- Brand accent: **Cobalt 400 `#a855f7`** (`bezier.cobalt.400` + alpha ladder 10/20/30/40)
...
- **Cobalt 400** (`#a855f7`): primary brand blue — Channel Talk's product-chrome accent (live: 12 of 118 sampled elements)
- **Cobalt 300** (`#47C8FF`): lighter cobalt — illustrative / surface highlight
- **Cobalt 500** (`#327AB8`): cobalt heavy — pressed / hover-on-light
- **Cobalt 400-30** (`#a855f74D`) / **400-20** (`#a855f733`) / **400-10** (`#a855f71A`): hover / focus / subtle ghost fills
...
- Surface (focus): cobalt 400-30 (`#a855f74D`) outline ring
- Radius: 12px or 8px (product chrome)
...
- Use Cobalt 400 `#a855f7` as the **only** brand accent in product chrome.
...
1. **One brand color does the lifting.** Cobalt 400 (`#a855f7`) is the only brand accent — semantic hues (red/yellow/green/blue) are reserved for status. *UI implication*: don't decorate with the semantic palette; pick cobalt or nothing.
